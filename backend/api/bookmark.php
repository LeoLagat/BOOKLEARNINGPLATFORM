...
if ($bookId <= 0) {
    header("Location: books.php?toast=bookmark_error");
    exit();
}
...
$stmt = $conn->prepare("SELECT id, membership_required FROM books WHERE id = ? LIMIT 1");
if (!$stmt) {
    header("Location: books.php?toast=bookmark_error");
    exit();
}
...
$existsStmt = $conn->prepare("SELECT id FROM user_bookmarks WHERE user_id = ? AND book_id = ? LIMIT 1");
if (!$existsStmt) {
    header("Location: books.php?toast=bookmark_error");
    exit();
}

$existsStmt->bind_param("ii", $userId, $bookId);
$existsStmt->execute();
$existsResult = $existsStmt->get_result();
$exists = $existsResult ? $existsResult->fetch_assoc() : null;

if ($exists) {
    $deleteStmt = $conn->prepare("DELETE FROM user_bookmarks WHERE user_id = ? AND book_id = ?");
    if (!$deleteStmt) {
        header("Location: books.php?toast=bookmark_error");
        exit();
    }
    ...
    $activityStmt = $conn->prepare("INSERT INTO user_book_activity (user_id, book_id, action) VALUES (?, ?, 'bookmark_remove')");
    if ($activityStmt) {
        $activityStmt->bind_param("ii", $userId, $bookId);
        $activityStmt->execute();
    }

    header("Location: books.php?toast=bookmark_removed");
    exit();
}

$insertStmt = $conn->prepare("INSERT INTO user_bookmarks (user_id, book_id) VALUES (?, ?)");
if (!$insertStmt) {
    header("Location: books.php?toast=bookmark_error");
    exit();
}
...
$activityStmt = $conn->prepare("INSERT INTO user_book_activity (user_id, book_id, action) VALUES (?, ?, 'bookmark_add')");
if ($activityStmt) {
    $activityStmt->bind_param("ii", $userId, $bookId);
    $activityStmt->execute();
}

header("Location: books.php?toast=bookmark_added");
exit();
